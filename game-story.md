...
_As built (issue #166): the arch stands OUTSIDE THE ENCAMPMENT — the act's staging
ground — sited like the Hold's on level ground of its own bearing, and it is the one
`GateSpec` with an `open` condition: stone, no light, a shut hint and no crossing until
the three `guardian-*-freed` flags are set. Pell gives and closes the quest AT THE FIRE
(`npc:sky-gardener/encampment`): the whole cast is home for the epilogue, and each of
them has a last line per ending, read off the ending flag. Rhune is one `enemy:rhune`
with `phases` — ground, water at 66%, flying at 33% — and the seam stage moves it to the
sector of each; `decide` is a `decide` trigger whose `choices` the engine's choice panel
shows at the lever, and each choice runs `ending.set`, the ONE handler that writes the
three ending flags, so exactly one is set by construction. `music:seam` is a track the
project generates itself (`bun tools/gen-seam-track.mjs`)._
...
- **`decide`** trigger + `choices` on an objective, the choice panel (`ui/choice.ts`)
  and the `ending.set` action — with #166.
...
- ~~Zone `seam`~~ — shipped with #166 (`world/seam.ts`), a floating disc of the
  three regions in one sky, entered through the one arch the player opens.
- `biome:reef`.
...
**Music.** `music:brine`, `music:cirrus` — composed tracks with their credits per the
licence rule in [AGENTS.md](AGENTS.md); `music:seam` is generated by the project's own
code (`tools/gen-seam-track.mjs`), so it is original work and needs no third-party
credit.
