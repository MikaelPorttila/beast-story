...
export const sections = [
  ...
  { id: 'census', run: async (ctx) => {
    ...
    if (!census) throw new Error('no census');
    ...
    ctx.check(grounded.length > 0, 'no ground towns in this world — the default arm cannot be tested');
    for (const t of perTown) {
      ...
      ctx.check(t.zone !== null && Math.abs(t.zone - (t.outerRadius + 6)) < 0.01,
    ...
    ctx.check(ctx.res.census.dens === 0,
      `${ctx.res.census.dens} skill dens claimed a zone — the default for a POI is none`);
    const gate = byId.get('landmark:gateway');
    ctx.check(!!gate, 'the zone gateway claimed no keep-out — the designer-set POI arm is untested');
    ctx.check(!gate || gate.radius === 12, `the gateway's keep-out is ${gate.radius}, expected 12`);
  ...
  // -------------------------------------------------------------------------
  { id: 'rim', run: async (ctx) => {
    // The query itself, at the rim.
    const r = seat.radius;
    const at = (d) => ctx.ev(([a, b]) => window.__dbgSafeZones?.(a, b).blocks, [seat.x + d, seat.z]);
    ctx.res.rim = {
    ...
    ctx.check(ctx.res.rim.outside === false,
      `${r + 0.5} from the middle still blocks — the disc is too wide`);
  } },

  // -------------------------------------------------------------------------
  { id: 'noSpawnInside', run: async (ctx) => {
    ...
    for (let i = 0; i < 60; i++) {
      ...
      for (const e of live) {
        ...
        if (e.d < seat.radius) {
          ...
          const seenBefore = prev.some((p) =>
            p.species === e.species && Math.hypot(p.x - e.x, p.z - e.z) < 5);
          if (seenBefore) ambledIn++;
          else appeared = { ...e, sample: i };
        }
      }
      prev = live;
    }
  ...
  { id: 'chase', run: async (ctx) => {
    ...
    ctx.check(Number.isFinite(best) && best < seat.radius,
