...
const setNature = (page, id, v, area) => page.evaluate(
  ([i, val, a]) => window.__dbgSetNature(i, val, a ?? undefined), [id, v, area ?? null]);
...
check(Object.values(base.baseline).every((v) => v === 1),
  `the shipped baseline is not all 1: ${JSON.stringify(base.baseline)}`);
check(base.census.chunks > 20 && base.census.grassVerts > 0 && base.census.propVerts > 0,
...
// rebuild itself. Everything below is read against this tolerance.
{
  ...
  const drift = Math.abs(same.census.grassVerts - base.census.grassVerts)
    / Math.max(1, base.census.grassVerts);
...
// mix of biomes and only the plains chunks in it are governed by the override.
{
  ...
  check(half.census.grassVerts < base.census.grassVerts * 0.85,
    `a 0.5 baseline barely thinned the sward (${half.census.grassVerts} against `
    + `${base.census.grassVerts})`);
  check(lifted.census.grassVerts > half.census.grassVerts,
    'plains x2 on a 0.5 baseline grew nothing back — the area is not layered on '
    + 'the baseline');
  check(lifted.census.grassVerts <= base.census.grassVerts * 1.05,
    'plains x2 on a 0.5 baseline overshot the full baseline — the area is '
    + 'replacing the baseline rather than multiplying it');
  check(snap.baseline.grass === 0.5 && snap.areas['plains.grass'] === 2,
...
// ---------- the URL says the same thing before the first chunk is built -----
{
  ...
  check(off.census.propVerts < base.census.propVerts * 0.6,
