...
export const sections = [
  // -------------------------------------------------------------------------
  {
    ...
    run: async (ctx) => {
      ...
      ctx.check(
        !!(await overlay(ctx)),
        "the F2 overlay did not open — no FPS readout for the cap section",
      );
    },
  },
  ...
  // -------------------------------------------------------------------------
  {
    ...
    run: async (ctx) => {
      ...
      ctx.check(
        shown?.rows ===
          ctx.res.panel.options + 1 + shown?.hairRows + shown?.pathRows + shown?.mountRows,
  ...
  // -------------------------------------------------------------------------
  {
    ...
    run: async (ctx) => {
      ...
      ctx.check(
        total(restored, "props") >= total(high, "props"),
  ...
  // -------------------------------------------------------------------------
  {
    ...
    run: async (ctx) => {
      ...
      for (let i = 0; i < 3 && !(await ctx.ev(() => !!document.querySelector(".bs-pause"))); i++) {
        await ctx.page.keyboard.press("F10");
        await ctx.adv(0.3);
        await ctx.waitFn(() => !!document.querySelector(".bs-pause"), 2000).catch(() => {});
      }
      ctx.check(
        await ctx.ev(() => !!document.querySelector(".bs-pause")),
        ...
        ctx.ev(() => {
          ...
          return (
            b && {
              ...
              pill: b.querySelector(".pill")?.textContent,
            }
          );
        });
      const rows = await ctx.ev(() =>
        [...document.querySelectorAll(".bs-pause [data-gfx]")].map((b) =>
          b.getAttribute("data-gfx"),
        ),
      );
      ...
      const foliageChoice = {
        ...
        pill: await ctx.ev(
          () =>
            document.querySelector('.bs-pause [data-gfx="foliageDistance"] .pill')?.textContent ??
            null,
        ),
      };
  ...
  // -------------------------------------------------------------------------
  {
    ...
    run: async (ctx) => {
      ...
      const stack = await ctx.ev(() => {
        ...
        const l = Math.max(c.left, p.left),
          r = Math.min(c.right, p.right);
        const t = Math.max(c.top, p.top),
          b = Math.min(c.bottom, p.bottom);
      ...
      ctx.check(
        stack.z.overlay !== null,
        "the F2 overlay was not found — the stage section opened it",
      );
      ctx.check(
        stack.z.console > stack.z.overlay,
  ...
  // -------------------------------------------------------------------------
  {
    ...
    run: async (ctx) => {
      ...
      try {
        ...
        ctx.res.preloadWalk = {
          gateBearing: +gateAt.bearing.toFixed(3),
          gateDist: +gateAt.dist.toFixed(1),
        };
        ...
        ctx.check(
          ctx.res.preloadWalk.gateDist < 10,
        ...
        for (const [when, snap] of [
          ["at rest", atRest],
          ["after walking", afterWalk],
        ]) {
          ctx.check(
            snap.terrain.hidden === 0 && snap.terrain.shown > 0,
      ...
      try {
        const decode = async () => {
          ...
          for (let i = 0; i < l.length; i++) {
            l[i] =
              0.2125 * png.data[i * 4] +
              0.7154 * png.data[i * 4 + 1] +
              0.0721 * png.data[i * 4 + 2];
          }
          return l;
        };
        ...
        const frameUp = async (camLook) => {
          ...
          for (let i = 0; i < 40; i++) {
            if (await shot.evaluate(() => !window.__dbgZone().streaming)) {
              break;
            }
        ...
        await shot.evaluate(() => {
          window.__dbgGfx("grass", true);
          window.__dbgGfx("props", false);
        });
        await settle();
  ...
  // -------------------------------------------------------------------------
  {
    ...
    run: async (ctx) => {
      ...
      ctx.check(
        at120 > at30 + 20,
        `raising the cap did not raise the frame rate (${at30} -> ${at120})`,
      );
    },
  },
