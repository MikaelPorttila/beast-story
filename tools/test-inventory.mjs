...
async function drag(ctx, fromSel, to) {
  ...
  if (!a || !b) {
    throw new Error(`nothing to drag: ${fromSel} -> ${JSON.stringify(to)}`);
  }
...
export const sections = [
  // ---------- 1. the starting kit, and the loadout it implies ----------
  {
    ...
    run: async (ctx) => {
      boot = await inv(ctx);
      if (!boot) {
        throw new Error("__dbgInventory is not there — nothing below can run");
      }
      ...
      ctx.check(
        boot.attackStat === boot.baseAttack + 4,
      ...
      ctx.check(
        boot.gear.find((g) => g.slot === "orb")?.id === "orb-tame",
      ...
      for (const id of GRANTED) {
        await ctx.ev((s) => window.__dbgGrantBeast(s), id);
      }
      boot = await inv(ctx);
      ...
      ctx.check(
        !boot.entries.some((e) => boot.gear.some((g) => g.id === e.id)),
        "a row is drawn both in a gear slot and on the wall",
      );
      ctx.check(
        !boot.bag.some((e) => e.id.startsWith("beast:")),
      ...
      const gearBeasts = boot.gear
        ...
        .map((g) => g.id);
      ctx.check(
        gearBeasts.every((id) => id && id.startsWith("beast:")),
  ...
  // down must move him — otherwise "0" is a hero who could not walk anyway.
  {
    ...
    run: async (ctx) => {
      ...
      await ctx.page
        .waitForFunction(
          () => {
            ...
            return (
              !window.__dbgZone().streaming && beasts.length > 0 && beasts.every((b) => !b.transit)
            );
          },
          { timeout: 30000 },
        )
      ...
      await ctx.ev(() =>
        window.__dbgTp?.(window.__dbgTowns().spawn.x, window.__dbgTowns().spawn.z),
      );
      ...
      ctx.check(
        open.panel?.filled === open.entries.length,
        `${open.panel?.filled} filled cells for ${open.entries.length} rows`,
      );
      ctx.check(
        open.entries.length + open.gear.filter((g) => g.id).length === ROSTER + boot.bag.length,
  ...
  // empty box that opens on hover would pass any "is it visible" test.
  {
    ...
    run: async (ctx) => {
      ...
      ctx.check(
        !!hovered.panel?.tip && hovered.panel.tip.includes("Mending"),
        `the tooltip reads ${JSON.stringify(hovered.panel?.tip)} — it should name the item`,
      );
      ctx.check(
        hovered.panel.tip.includes("Restores"),
  ...
  // ---------- 3. equip and unequip move the stat ----------
  {
    ...
    run: async (ctx) => {
      ...
      ctx.check(
        !armed.entries.some((e) => e.id === "sword-iron"),
        "the equipped sword is on the wall as well as in the weapon slot",
      );
      ctx.check(
        !row.actions.includes("drop") && !row.actions.includes("salvage"),
    ...
    run: async (ctx) => {
      ...
      const rmb = (sel) =>
        ctx.ev((s) => {
          ...
          if (!el) {
            throw new Error(`no box holds ${s}`);
          }
  ...
  // to be over the thing being asserted about.
  {
    ...
    run: async (ctx) => {
      ...
      await ctx.waitFn(
        () => !!document.querySelector('.bs-inv .slot[data-sel="greatsword-iron"]'),
        5000,
      );
      ...
      await drag(
        ctx,
        '.bs-inv .slot[data-sel="greatsword-iron"]',
        '.bs-inv .gs[data-gear="weapon"]',
      );
      ...
      await drag(
        ctx,
        `.bs-inv .slot[data-sel="${benched.id}"]`,
        '.bs-inv .gs[data-gear="primary"]',
      );
      ...
      await ctx.waitFn(() => !!document.querySelector('.bs-inv .slot[data-sel="sunberry"]'), 5000);
  ...
  // predict), and a bag CHANGING under a moved row without shuffling it back.
  {
    ...
    run: async (ctx) => {
      ...
      await drag(
        ctx,
        '.bs-inv .slot[data-sel="potion-mend"]',
        `.bs-inv .slot[data-sel="${neighbour}"]`,
      );
      ...
      ctx.check(
        ctx.res.arrange.giftAt >= 0 && ctx.res.arrange.giftAt !== parked,
  ...
  // roster, so each half is put back and re-read.
  {
    ...
    run: async (ctx) => {
      ...
      ctx.check(
        alone.entries.filter((e) => e.kind === "beast").length === ROSTER,
  ...
  // panel has to still be there afterwards.
  {
    ...
    run: async (ctx) => {
      ...
      const click = async (sel) => {
        ...
        if (!p) {
          throw new Error(`nothing at ${sel}`);
        }
  ...
  // ---------- 4. using a potion: hp up, stack down, buff on a clock ----------
  {
    ...
    run: async (ctx) => {
      ...
      ctx.check(
        hurt.hp < 100,
        `the hero is on ${hurt.hp} hp — nothing to heal, section 4 proves nothing`,
      );
      ctx.check(healed.hp > hurt.hp, `hp did not move: ${hurt.hp} -> ${healed.hp}`);
      ctx.check(
        ctx.res.use.potionsAfter === ctx.res.use.potionsBefore - 1,
      ...
      ctx.check(
        furious.buff.attack === 10 && furious.buff.seconds > 0,
      ...
      ctx.check(
        furious.attackStat === gotFury.attackStat + 10,
  ...
  // ---------- 6. the control: a quest item refuses both ----------
  {
    ...
    run: async (ctx) => {
      ...
      ctx.check(
        ctx.res.quest.countAfter === ctx.res.quest.countBefore,
        "a quest item was destroyed by a direct call to the action handler",
      );
      ctx.check(
        ctx.res.quest.shardsAfter === ctx.res.quest.shardsBefore,
  ...
  // screen does not.
  {
    ...
    run: async (ctx) => {
      ...
      const swing = async () => {
        ...
        for (let i = 0; i < 5; i++) {
          if (!(await shots()).shots.some((s) => s.arrow)) {
            break;
          }
          await ctx.adv(0.5);
        }
