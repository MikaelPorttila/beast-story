...
// ---------- 1. the offer, and only on the giver -----------------------------
{
  ...
  check(m.drawn.some((d) => d.kind === 'offer'),
...
// ---------- 3. what the quest counts is marked ------------------------------
{
  ...
  results.target = {
    beasts: m.marked.beasts, enemies: m.marked.enemies,
    targetsDrawn: m.drawn.filter((d) => d.kind === 'target').length,
    wildNearby: wild.map((e) => e.species),
  };
  check(m.marked.beasts.includes('sproutle') || m.marked.enemies.includes('wild-sproutle'),
  ...
  check(results.target.targetsDrawn <= wild.length - unmarked + 1,
...
// ---------- 4. done, and the mark comes back as a turn-in -------------------
{
  ...
  await page.waitForFunction(
    () => window.__dbgQuestMarks().marked.npcs.some((n) => n.kind === 'turnIn'),
    { timeout: 20000 },
  ).catch(() => {});
  const m = await marks();
  results.turnIn = { marked: m.marked, drawn: m.drawn };
  check(await kindOn('gain') === 'turnIn',
  ...
  check(m.marked.beasts.length === 0 && m.marked.enemies.length === 0,
