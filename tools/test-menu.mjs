...
const state = (page) => page.evaluate(() => {
  ...
  return {
    ...
    buttons: [...document.querySelectorAll('.bs-menu .panel button')]
      .map((b) => b.dataset.act ?? b.dataset.toggle ?? b.dataset.gfx ?? b.dataset.lang ?? b.dataset.tab ?? '?'),
...
const focusRing = (page) => page.evaluate(() => {
  ...
  return {
    on: a.dataset.act ?? a.dataset.toggle ?? a.dataset.gfx ?? a.dataset.lang ?? a.dataset.tab ?? '?',
...
// ---- desktop: gate, then release ------------------------------------------
{
  ...
  // ring rather than the plain one. Four steps down is past the four tabs and
  // onto the first row; one more is the language chip below it.
  ...
  // The four sections, and which one a fresh panel opens on.
