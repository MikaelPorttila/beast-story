...
export const sections = [
  ...
  // -------------------------------------------------------------------------
  {
    ...
    run: async (ctx) => {
      ...
      geom = await ctx.ev(() => {
        ...
        for (let dx = -220; dx <= 220; dx += 2) {
          for (let dz = -220; dz <= 220; dz += 2) {
            if (!window.__dbgWorld(s.x + dx, s.z + dz).deep) {
              continue;
            }
            const d = dx * dx + dz * dz;
            if (d < dbest) {
              dbest = d;
              deep = { x: s.x + dx, z: s.z + dz };
            }
          }
        }
        if (!deep) {
        ...
        for (let dx = -100; dx <= 100; dx++) {
          for (let dz = -100; dz <= 100; dz++) {
            ...
            if (d < sbest) {
              sbest = d;
              shore = { x: deep.x + dx, z: deep.z + dz, ground: w.ground };
            }
          }
        }
      ...
      if (!geom) {
        throw new Error("no geometry to drive at");
      }
  ...
  // -------------------------------------------------------------------------
  {
    ...
    run: async (ctx) => {
      ...
      const run = await drive(ctx, bx, bz, yaw);
      ctx.check(
        Math.hypot(replaced.x - bx, replaced.z - bz) < 3,
      ...
      ctx.check(
        run.topSpeedOnWater > landRun.topSpeedOnLand * 1.8,
  ...
  // -------------------------------------------------------------------------
  {
    ...
    run: async (ctx) => {
      ...
      await ctx.adv(1.5);
      const bed = await ctx.ev(
        ([x, z]) => window.__dbgWorld(x, z).ground,
      ...
      ctx.check(
        rise.end.depth < dive.end.depth - 1,
      ...
      const breach = [...dive.track, ...rise.track, ...sink.track, ...bob.track].find(
        (s) => s.y > float.bodyY + 0.05,
      );
      ctx.check(
        !breach,
