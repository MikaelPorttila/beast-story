...
export const sections = [
  ...
  // -------------------------------------------------------------------------
  {
    ...
    run: async (ctx) => {
      ...
      if (!island) {
        throw new Error("nothing to test against");
      }
      ctx.res.carrier = {
        id: island.id,
  ...
  // -------------------------------------------------------------------------
  {
    ...
    run: async (ctx) => {
      ...
      ctx.check(
        !!spot,
        "no bare turf column found on the 0.62 r ring — is the deck all buildings?",
      );
  ...
  // -------------------------------------------------------------------------
  {
    ...
    run: async (ctx) => {
      ...
      ctx.check(
        moved < ctx.res.underneath.islandMoved * 0.35,
  ...
  // -------------------------------------------------------------------------
  {
    ...
    run: async (ctx) => {
      ...
      for (let i = 0; i < offsets.length; i++) {
        ctx.check(
          Math.abs(offsets2[i] - offsets[i]) < 1.0,
  ...
  // -------------------------------------------------------------------------
  {
    ...
    run: async (ctx) => {
      ...
      ctx.check(
        m0.mounted && m0.locomotion === "flying",
  ...
  // -------------------------------------------------------------------------
  {
    ...
    run: async (ctx) => {
      ...
      const depth =
        here.keel !== null && here.surface > here.keel
      ...
      ctx.res.shoved = {
        ...
        fromCentre: +staged.toFixed(2),
        toCentre: +d1.toFixed(2),
        radius: c1.radius,
        y: m1.bodyY,
        surface: c1.surface,
        keel: c1.keel,
      ...
      ctx.check(
        !(m1.bodyY > c1.surface && c1.surface !== null),
  ...
  // -------------------------------------------------------------------------
  {
    ...
    run: async (ctx) => {
      ...
      ctx.res.landed = {
        attached: attached.riding,
        riding: s.riding,
        y: m.bodyY,
        surface: c.surface,
        over: +(m.bodyY - c.surface).toFixed(2),
      };
      ctx.check(
        attached.riding === island.id,
  ...
  // -------------------------------------------------------------------------
  {
    ...
    run: async (ctx) => {
      ...
      ctx.check(st.paved > 1000, `only ${st.paved} voxel cells were painted as flagstone`);
      ctx.check(
        st.clear.every((d) => d >= 0),
