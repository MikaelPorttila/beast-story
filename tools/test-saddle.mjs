...
const groundAt = (x, z) => page.evaluate(([a, b]) => window.__dbgSurfaceY(a, b).ground, [x, z]);
...
// ---------- in the air: he stays at the saddle and keeps falling ----------
{
  ...
  check(
    Math.abs(landed.y - (await groundAt(landed.x, landed.z))) < 2,
...
// used to assign the floor outright and land the pair of them instantly.
{
  ...
  check(
    Math.abs(landed.bodyY - ground) < 2,
