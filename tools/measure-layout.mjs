...
for (const [name, viewport] of [
  ...
  out[name] = await page.evaluate(() => {
    ...
    return {
      ...
      crossCentre: crossR
        ? {
            x: +(crossR.x + crossR.width / 2).toFixed(1),
            y: +(crossR.y + crossR.height / 2).toFixed(1),
          }
        : null,
      ...
      nearestControlToCross: (() => {
        ...
        const cx = crossR.x + crossR.width / 2,
          cy = crossR.y + crossR.height / 2;
        let best = Infinity,
          who = null;
        for (const el of document.querySelectorAll(".bs-btn,.bs-skill,.bs-stick")) {
          ...
          if (d < best) {
            best = d;
            who = el.className + ":" + el.textContent;
          }
        }
