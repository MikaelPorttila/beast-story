...
if (before) {
  check(
    before.terrainVertices > 1_000 && before.terrainVertices < 20_000,
    `far terrain is not a coarse consolidated mesh: ${before.terrainVertices} vertices`,
  );
  check(
    before.waterVertices === before.terrainVertices,
    `terrain/water clipmaps disagree: ${JSON.stringify(before)}`,
  );
  check(before.wetWaterVertices > 0, "far water sampled no wet vertices from the terrain field");
  check(before.step >= 8, `far terrain kept near-detail resolution (${before.step}m samples)`);
  check(
    before.outerRadius > 600,
    `far terrain can end inside the camera plane (${before.outerRadius}m)`,
  );
  check(
    before.outerRadius >= before.viewDistance + 64,
    `far terrain has no snap-cell reserve: ${JSON.stringify(before)}`,
  );
  check(
    before.innerRadius < 160,
    `far terrain does not overlap the streamed rim (${before.innerRadius}m)`,
  );
  check(
    before.ready === true && before.building === false,
    `play began with an incomplete far underlay: ${JSON.stringify(before)}`,
  );
  check(
    before.waterFadeEnd === before.innerRadius + 32 &&
  ...
  check(
    high.viewDistance === 900 && high.outerRadius > 900,
    `High did not extend the rendered horizon: ${JSON.stringify(high)}`,
  );
  check(
    high.innerRadius === before.innerRadius,
    `High queued extra cube-detail terrain: ${before.innerRadius} -> ${high.innerRadius}`,
  );
  check(
    high.step < before.step && high.terrainVertices > before.terrainVertices * 3,
  ...
  check(
    low.viewDistance === 480 && low.outerRadius > 480,
    `Low can expose the far mesh edge: ${JSON.stringify(low)}`,
  );
  check(
    low.step > before.step && low.terrainVertices < before.terrainVertices,
  ...
  for (let i = 0; i < 40; i++) {
    ...
    if (after?.anchor?.[0] !== before.anchor[0]) {
      break;
    }
  }
  check(
    after.anchor[0] - before.anchor[0] >= 128,
    `clipmap did not follow the player: ${before.anchor} -> ${after.anchor}`,
  );
  check(
    after.terrainVertices === before.terrainVertices,
