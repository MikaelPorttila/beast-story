...
const volKey = (page) => page.evaluate(() => localStorage.getItem("game.settings.gameplay.volume"));
...
const booted = (page) =>
  page.waitForFunction(() => typeof window.__dbgMusic === "function", { timeout: 60000 });
...
// ---- 2/3. vol= wins, the track plays, and the head fades in ----------------
{
  ...
  check(
    out.wrapped?.output < 0.45,
    `the head after a wrap fades in too, got ${out.wrapped?.output}`,
  );
  await ctx.close();
}
...
// ---- 4/5/6. the scene changes, and the volume row --------------------------
{
  ...
  await page.waitForFunction(() => /overworld/.test(window.__dbgMusic()?.track ?? ""), {
    timeout: 60000,
  });
  await wait(1500);
  ...
  out.chips = await page.evaluate(() =>
    [...document.querySelectorAll(".bs-pause [data-vol]")].map((b) => ({
      v: b.getAttribute("data-vol"),
  ...
  await page.waitForFunction(() => /title/.test(window.__dbgMusic()?.track ?? ""), {
    timeout: 20000,
  });
  out.backAtTitle = await music(page);
  check(out.backAtTitle?.scene === "title", "Exit goes back to the title track");
  check(
    out.backAtTitle?.starts === 4,
    `four tracks over the session, got ${out.backAtTitle?.starts}`,
  );
  await ctx.close();
}
...
// `__dbgMusicScene` rather than a walk to the gateway: see the hook in main.ts.
{
  ...
  await page.waitForFunction(() => window.__dbgMusic()?.loaded === false, { timeout: 8000 });
