...
import {
  pathProfile, ROAD_PROFILE, FOOTPATH_PROFILE, MAX_CARVE_BLEND,
} from '../src/world/path-profile.ts';

const fail = [];
const near = (a, b) => Math.abs(a - b) < 1e-9;
const eq = (what, got, want) => {
  if (!near(got, want)) fail.push(`${what}: ${got}, expected ${want}`);
};
const ok = (what, cond) => {
...
for (let i = 0; i < XS.length; i++) eq(`road xs[${i}]`, r.xs[i], XS[i]);
