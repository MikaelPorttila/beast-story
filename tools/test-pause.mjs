...
const canvasPoint = async (page) => {
  const p = await page.evaluate(() => {
    ...
    for (const [x, y] of [
      ...
      if (document.elementFromPoint(x, y) === cvs) {
        return { x, y };
      }
    }
    return null;
...
export const sections = [
  ...
  // -------------------------------------------------------------------------
  {
    ...
    run: async (ctx) => {
      ...
      m.rows = await ctx.ev(() =>
        [...document.querySelectorAll(".bs-pause [data-act]")].map((b) =>
          b.getAttribute("data-act"),
        ),
      );
      // A pad player has no mouse: something has to be under the cursor on open.
      m.focusOnOpen = await ctx.ev(() => document.activeElement?.getAttribute("data-act") ?? null);
  ...
  // -------------------------------------------------------------------------
  {
    ...
    run: async (ctx) => {
      ...
      const settings = await ctx.ev(() => ({
        tabs: [...document.querySelectorAll(".bs-pause [data-tab]")].map((b) =>
        ...
        toggles: [...document.querySelectorAll(".bs-pause .sec:not(.off) [data-toggle]")].map((b) =>
          b.getAttribute("data-toggle"),
        ),
        langChips: document.querySelectorAll(".bs-pause [data-lang]").length,
        langAllDisabled: [...document.querySelectorAll(".bs-pause [data-lang]")].every(
          (b) => b.disabled,
        ),
        ...
        return ctx.ev(() => {
          ...
          return {
            ...
            strayFocusable: document.querySelectorAll(
              '.bs-pause .sec.off button:not([disabled]):not([tabindex="-1"]):not(.sec.off *)',
            ).length,
      ...
      ctx.check(
        settings.langChips > 0 && settings.langAllDisabled,
      ...
      ctx.check(
        JSON.stringify(tabControls.toggles) ===
      ...
      ctx.check(
        JSON.stringify(tabGraphics.gfx) ===
          JSON.stringify([
            "ao",
            "bloom",
            "aa",
      ...
      for (const [tname, t] of Object.entries({
        ...
        sound: tabSound,
        gameplay: tabGameplay,
      })) {
        ctx.check(t.lit === tname, `the ${tname} tab lights when it is pressed (lit ${t.lit})`);
        ctx.check(
          t.focus === tname,
          `and keeps the cursor on itself through the rebuild (${t.focus})`,
        );
        ctx.check(
          t.strayFocusable === 0,
  ...
  // -------------------------------------------------------------------------
  {
    ...
    run: async (ctx) => {
      ...
      ctx.check(
        escapeFromSettings.stillOpen && escapeFromSettings.backOnTheList,
  ...
  // -------------------------------------------------------------------------
  {
    ...
    run: async (ctx) => {
      if (!lockGranted) {
        ctx.res.unfocused = { skipped: "no pointer lock" };
        return;
      }
      const { page } = ctx;
      const u = {};
      ...
      const relock = async () => {
        ...
        for (let attempt = 0; attempt < 3; attempt++) {
          ...
          const got = await page
            .waitForFunction(() => document.pointerLockElement !== null, { timeout: 1200 })
            .then(() => true)
            .catch(() => false);
          if (got) {
      ...
      await page
        .waitForFunction(() => window.__dbgInput().relockPending === true, { timeout: 2500 })
        .catch(() => {});
  ...
  // -------------------------------------------------------------------------
  {
    ...
    run: async (ctx) => {
      ...
      try {
        ...
        await sleep(1000);
        exit.lockAtTitle = await page.evaluate(() => document.pointerLockElement?.tagName ?? null);
        ...
        await page
          .waitForFunction(() => !document.querySelector(".bs-menu"), { timeout: 8000 })
          .catch(() => {});
        ...
        ctx.res.exit = exit;
        ctx.check(
          exit.playingFirst && exit.travelFirstGame > 4,
        ...
        ctx.check(
          exit.lockAtTitle === null,
          `the pointer is given back at the title screen (locked to ${exit.lockAtTitle})`,
        );
        ctx.check(
          exit.secondGame.menuGone,
          "New Game works a second time, from a real mouse click",
        );
        ...
        ctx.check(
          exit.posterRemnants.flies === 0 && exit.posterRemnants.lamps === 0,
