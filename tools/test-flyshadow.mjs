...
const groundAt = (x, z) =>
  page.evaluate(([a, b]) => window.__dbgSurfaceY(a, b), [x, z]);
/**
...
// ---------- high above it: no disc in the sky ----------
{
  ...
  const p2 = await pos();
  results.high = b
    ? { heroY: p2.y, alt: b.altitude, blobY: b.blob.y, opacity: b.blob.opacity, visible: b.blob.visible }
    : null;
  ...
  if (b) {
    ...
    check(!b.blob.visible || b.blob.opacity < 0.01,
