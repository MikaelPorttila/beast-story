...
const setHair = (style, colour) => page.evaluate(
  ([s, c]) => window.__dbgHair(s, c), [style, colour]);
...
check(duplicates === 0, `${duplicates} styles share a vertex count — two of them are the same head`);
...
out.colour = { green: { colour: green.colour, tint: green.tint }, red: { colour: red.colour, tint: red.tint } };
check(green.colour === '20d060', `colour not stored: ${green.colour}`);
check(green.tint[1] > green.tint[0] && green.tint[1] > green.tint[2], 'a green head is not green in the mesh');
...
check(green.vertices === shapes.classic.vertices,
...
check(cleared.colour === shapes.classic.colour,
