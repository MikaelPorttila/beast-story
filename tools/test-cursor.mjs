...
// there is a window where the menu is showing and nothing can be asked about it.
{
  ...
  // is here to make.
  await page
    .waitForFunction(() => window.__dbgCursor?.().free === true, { timeout: 15000 })
    .catch(() => {});
  const c = await page.evaluate(() => window.__dbgCursor());
  const btn = await page.evaluate(() => {
    const el =
      document.querySelector('.bs-menu [data-act="new"]') ??
  ...
  if (btn) {
    ...
    check(
      over.state === "link-select" || over.state === "default",
...
// ---------- the sheet is there and every state was cut ----------------------
{
  ...
  check(c.states === c.known, `${c.states} of ${c.known} cursor states were cut from the sheet`);
...
// no idea why.
{
  ...
  results.altHold = {
    lockedBefore: before,
    lockedWhileHeld: heldLock,
    lockedAfterRelease: afterLock,
    freeWhileHeld: heldCur.free,
    freeAfterRelease: afterCur.free,
  };
  ...
  check(
    afterCur.free === false,
    "the cursor stayed free after Alt was released — that is a toggle",
  );
}
...
// with. Nothing is held here — that is the point. F10 both opens and closes it.
{
  ...
  results.pauseMenu = {
    freeInMenu: inMenu.free,
    lockInMenu: menuLock,
    freeAfterClosing: afterMenu.free,
  };
...
// ---------- the resolver answers for real contexts --------------------------
{
  ...
  if (row) {
    cases["link-select"] = (await at(row.x, row.y)).state;
  }
  // Its title bar is what you pick the panel up by.
  const title = await box(".bs-perf-title");
  if (title) {
    cases.grab = (await at(title.x, title.y)).state;
  }
  ...
  for (const [sel, want] of [
    ...
    if (b) {
      cases[want] = (await at(b.x, b.y)).state;
    }
  }
...
// and reading them together on a narrow window means moving one.
{
  ...
  check(
    after.x - before.x > 150 && after.y - before.y > 100,
