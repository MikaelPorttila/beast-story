...
if (before === true) {
  fails.push("the overlay was already up before F2 was pressed");
}
if (afterOn !== true) {
  fails.push("F2 did not raise the overlay");
}
if (!/FPS[^\d]*\d/.test(text ?? "")) {
  fails.push(`the readout printed no rate: ${text}`);
}
if (afterOff !== false) {
  fails.push("a second F2 did not put the overlay away");
}
if (leaked) {
  fails.push("the browser saw an unprevented F2 — its own dev tools would open");
}
  ...
  JSON.stringify(
    {
      ...
      afterOff,
      readout: text,
      browserSawUnpreventedF2: leaked,
      pass: fails.length === 0,
    },
    null,
    2,
