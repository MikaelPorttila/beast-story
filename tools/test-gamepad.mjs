...
export const sections = [
  ...
  // -------------------------------------------------------------------------
  {
    ...
    run: async (ctx) => {
      ...
      ctx.check(
        ctx.res.noPad.keepsExistingKeys,
  ...
  // -------------------------------------------------------------------------
  {
    ...
    run: async (ctx) => {
      ...
      for (let i = 0; i < 15; i++) {
        if ((await yawOverSim(ctx.page, 0.2)) < 0.002) {
          break;
        }
      }
      ...
      ctx.check(
        ctx.res.deadzone.stick?.fwd === 0 && ctx.res.deadzone.stick?.side === 0,
  ...
  // -------------------------------------------------------------------------
  {
    ...
    run: async (ctx) => {
      ...
      ctx.check(
        ctx.res.buttons.skillPressSeen,
    ...
    run: async (ctx) => {
      ...
      try {
        ...
        const hurt = () =>
          page.evaluate(() => {
            const before = {
              drained: window.__dbgFeedback().drained,
              rumble: window.__rumble.calls,
            };
            window.__dbgHurt(10);
  ...
  // -------------------------------------------------------------------------
  {
    ...
    run: async (ctx) => {
      ...
      const bctx = await ctx.page.browser().createBrowserContext();
      try {
        const page = await bootPadPage(bctx, "Xbox Wireless Controller", {
          query: "menu=0&fs=0&invy=0",
        });
        ...
        ctx.check(
          ctx.res.invertOverride.reportedInvertY === false,
          `?invy=0 left invertLookY at ${ctx.res.invertOverride.reportedInvertY}`,
        );
        ctx.check(
          ctx.res.invertOverride.stickUpLooksUp,
  ...
  // -------------------------------------------------------------------------
  {
    ...
    run: async (ctx) => {
      ...
      try {
        ...
        const hotbar = () =>
          page.evaluate(() => document.querySelector(".bs-slot .key")?.textContent);
        const padCaps = () =>
          page.evaluate(() => document.querySelectorAll(".bs-root kbd.pad").length);
        const source = async () => (await probe(page, "__dbgInput"))?.lastSource;

        const beforeUse = {
          hotbar: await hotbar(),
          padCaps: await padCaps(),
          source: await source(),
        };
        ...
        ctx.check(
          ctx.res.glyphs.dualsense === "playstation",
  ...
  // -------------------------------------------------------------------------
  {
    ...
    run: async (ctx) => {
      ...
      try {
        ...
        ctx.check(
          ctx.res.lookRateVsFrameRate.frameRateIndependent,
