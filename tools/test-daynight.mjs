...
try {
  ...
  await page.waitForFunction(
    () => window.__dbgBoot?.().playing && window.__dbgTime,
    { timeout: 60000 },
  );
  ...
  check(midnight.shadow.bounceIntensity >= 0.35,
    'midnight anti-moon fill is too weak to keep the hero face readable');
  check(midnight.lighting['hero-highlight'] === initial.lighting['hero-highlight']
  ...
  check(midnight.lighting['skyhaven-local-light'] >= 79,
    'Skyhaven fixtures emitted no local illumination at night');
  check(midnight.lighting['cloud-moon-fill'] > initial.lighting['cloud-moon-fill'],
    'clouds did not gain moon-scattered fill at night');
  const dot = midnight.sunDirection.reduce(
    (sum, v, i) => sum + v * midnight.moonDirection[i], 0,
  );
  ...
  check(cadence1 - cadence0 >= 1 && cadence1 - cadence0 <= 3,
  ...
  check(quest.source === 'quest' && quest.quest === 'quest:encampment/first-steps',
  ...
  check(debugOverQuest.source === 'debug' && circular(debugOverQuest.phase, 0.5) < 0.01,
    'debug preset did not override the quest lock');
  check(debugOverQuest.lighting['town-windows'] < 0.01
  ...
  check(questRestored.source === 'quest' && circular(questRestored.phase, 0) < 0.01,
  ...
  console.log(JSON.stringify({ initial, midnight, cadenceUpdates: cadence1 - cadence0,
    quest: questRestored, released, f3, failures }, null, 2));
} finally {
  await ctx.close();
  await browser.close();
}
