 ...
 * itself knows nothing about trees. It owns three things:
 *
 ...
 *     cheap contact test plus the look and physics of what it sheds.
 *
 ...
 * is skipped while a particle is still rising.
 */
...
// ---------------------------------------------------------------------------
// Element 1 of 1: leaves off a tree crown
// ---------------------------------------------------------------------------
...
const _point: ContactPoint = {
  x: 0, y: 0, z: 0, spread: 0, dirX: 0, dirZ: 0, color: new THREE.Color(),
};
...
export class TouchParticles implements WorldBound {
  ...
  /** Which SOURCES entry threw this particle — the kind to simulate it with. */
  private src = new Uint8Array(MAX);
  /**
  ...
  private acquire(): number {
    ...
    if (oldest >= 0) {
      const k = SOURCES[this.src[oldest]].kind;
  ...
  /** One particle of `s` at `pt`. False when the pool refused it. */
  private spawn(s: number, pt: ContactPoint): boolean {
    const i = this.acquire();
    if (i < 0) return false;
    const k = SOURCES[s].kind;
    ...
    // tree's leaves are a family rather than 64 copies of one swatch.
    const v = 0.86 + Math.random() * 0.28;
    _col.copy(pt.color).multiplyScalar(v);
    this.mesh.setColorAt(i, _col);
  ...
  private probeContacts(dt: number, mover: ContactMover): void {
    ...
    for (let s = 0; s < SOURCES.length; s++) {
      ...
      if (this.wasTouching[s] === 0) {
        ...
        for (let n = 0; n < src.onset; n++) if (!this.spawn(s, _point)) break;
      }
      const brush = Math.min(1, speed / src.brushSpeed);
      this.acc[s] += src.rate * brush * dt;
      while (this.acc[s] >= 1) {
        this.acc[s] -= 1;
        if (!this.spawn(s, _point)) break;
      }
  ...
  private integrate(dt: number): void {
    ...
    for (let i = 0; i < MAX; i++) {
      ...
      if (st === FREE) continue;
      const k = SOURCES[this.src[i]].kind;
      let s = this.size[i];
  ...
  forceBurst(mover: ContactMover, n: number): number {
    ...
    let placed = 0;
    for (let i = 0; i < n; i++) if (this.spawn(SOURCES.indexOf(src), _point)) placed++;
    return placed;
  }
