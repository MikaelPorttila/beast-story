...
function makeHead(): THREE.Mesh {
  ...
  eyes2x2(m, {
    inner: 1, width: 1, y: 3, faceZ: 2, iris: IRIS, shine: SHINE,
    lid: BACK_DARK, bridge: FLANK,
  });
  return m.build(S, true);
}
...
function buildRig(): BeastRig {
  ...
  dorsal.position.set(0, 0.38, -0.10);
  body.add(dorsal);
  ...
  const mkPect = (dir: number): THREE.Group => {
    ...
    g.position.set(dir * 0.20, -0.16, 0.16);
    g.rotation.set(0, dir * -0.35, dir * -0.30);
    body.add(g);
  ...
  tailTip.position.set(0, -0.02, -0.30);
  tailBase.add(tailTip);
  const fluke = makeFluke();
  fluke.position.set(-0.05, -0.02, -0.10);
  tailTip.add(fluke);
...
function animate(rig: BeastRig, ctx: BeastAnimCtx): void {
  ...
  switch (ctx.action) {
    case 'idle': {
      ...
      bsy = 1 + 0.02 * br;
      hry = 0.20 * Math.sin(t * 0.29);
      hrx = 0.06 * Math.sin(t * 1.1);
      hrz = 0.05 * Math.sin(t * 0.7);
      tbx = 0.10 * Math.sin(hover - 0.9);
    ...
    case 'cast': {
      ...
      hrz = 0.04 * song * rise;
      tbx = 0.45 * rise;
      ttx = 0.30 * rise + 0.10 * Math.sin(t * 8) * rise;
      pSweep = 0.45 * rise;
      pLift = -0.55 * rise + 0.12 * Math.sin(t * 7) * rise;
      dorsalTilt = 0.10 * Math.sin(t * 5) * rise;
      break;
    }
    case 'special': {
      ...
      const splash = Math.sin(Math.PI * phase(at, T, T + 0.26))
  ...
  p.pectR.rotation.set(0, -0.35 + pSweep, -0.30 - pLift + pSplit);
  p.pectL.rotation.set(0, 0.35 - pSweep, 0.30 + pLift + pSplit);
}
