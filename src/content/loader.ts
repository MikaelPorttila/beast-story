...
function isConditionShape(v: unknown, depth = 0): v is Condition {
  if (depth > 64 || !isRecord(v)) {
    return false;
  }
  if (Array.isArray(v.all)) {
    return v.all.every((c) => isConditionShape(c, depth + 1));
  }
  if (Array.isArray(v.any)) {
    return v.any.every((c) => isConditionShape(c, depth + 1));
  }
  if ("not" in v) {
    return isConditionShape(v.not, depth + 1);
  }
...
function readText(v: unknown): ContentText | null {
  ...
  if (typeof v.key === "string" && v.key !== "") {
    return { key: v.key } as ContentText;
  }
  if (isRecord(v.text)) {
...
export class PackageLoader {
  ...
  private ingest(
    ...
    const authored =
      typeof body.schema === "number" && Number.isFinite(body.schema) ? body.schema : def.schema;
  ...
  private hold(entry: LoadedPackage, hold: Hold): void {
    if ("lease" in hold) {
      entry.leases.add(hold.lease);
    } else {
      entry.dependents.add(hold.dependent);
    }
  }
  ...
  private collect(pkg: PackageId): void {
    ...
    if (entry.leases.size > 0 || entry.dependents.size > 0) {
      return;
    }
