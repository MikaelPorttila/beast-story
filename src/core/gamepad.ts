...
export class GamepadControls {
  ...
  get connected(): boolean {
    return this.padIndex >= 0 && this.pad !== null;
  }
  ...
  poll(dt: number): void {
    ...
    const looking = shape(
      pad.axes[2] * this.lookSignX,
      pad.axes[3] * this.lookSignY,
      DZ_LOOK,
      EXPO_LOOK,
    );
    if (looking) {
    ...
    if (force || held(B_RT) !== !!this.prev[B_RT]) {
      this.input.setVirtualAttack(held(B_RT));
    }
  ...
  private hold(i: number, code: string, held: (i: number) => boolean, force = false): void {
    const now = held(i);
    if (force || now !== !!this.prev[i]) {
      this.input.setVirtualButton(code, now);
    }
  }

  private edge(i: number, fn: () => void, held: (i: number) => boolean): void {
    if (held(i) && !this.prev[i]) {
      fn();
    }
  }
  ...
  debugState(): unknown {
    return {
      ...
      pressed: this.pad
        ? this.pad.buttons
            .map((x, i) => (x.pressed || x.value > TRIGGER_ON ? i : -1))
            .filter((i) => i >= 0)
        : [],
    };
  }
