...
export const GFX_OPTIONS: readonly GfxOption[] = [
  {
    id: "fpsCap",
    labelKey: "gfx.fpsCap",
    costKey: "gfx.fpsCap.cost",
    choices: [0, 30, 60, 120, 144],
    def: 120,
  },
  ...
  {
    id: "terrainDistance",
    labelKey: "gfx.terrainDistance",
    costKey: "gfx.terrainDistance.cost",
    choices: [480, 600, 900],
    def: 600,
  },
  ...
  {
    id: "foliageDistance",
    labelKey: "gfx.foliageDistance",
    costKey: "gfx.foliageDistance.cost",
    choices: [64, 96, 128],
    def: 128,
  },
...
function writeRaw(id: string, value: string | null): void {
  try {
    ...
    /* storage denied — the session still honours the value */
  }
}
...
export class Gfx {
  ...
  constructor(private readonly sinks: GfxSinks) {
    for (const o of GFX_OPTIONS) {
      this.values.set(o.id, parse(o, readRaw(o.id)));
    }
  }
  ...
  set(id: keyof GfxSinks, value: GfxValue): GfxValue {
    ...
    const v: GfxValue = opt.choices
      ? opt.choices.includes(Number(value))
        ? Number(value)
        : opt.def
      : Boolean(value);
