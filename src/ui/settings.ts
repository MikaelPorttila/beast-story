 ...
 */
export const FOCUSABLE = 'button:not([disabled]):not([tabindex="-1"]):not(.sec.off *)';
...
export class SettingsPanel {
  ...
  markup(): string {
    return `<h2>${escapeHtml(t("menu.settings.title"))}</h2>` + this.tabStrip() + this.sections();
  }
  ...
  private tabStrip(): string {
    return `<div class="tabs strip" role="tablist" data-group="tab">${TABS.map((tb) => {
      ...
      return (
        `<button class="bs-menu-btn chip tab${on ? " on" : ""}" type="button" ` +
  ...
  private sections(): string {
    ...
    return (
      ...
      this.section(
        ...
        this.toggle(
          "autoFullscreen",
          t("menu.settings.autoFullscreen"),
          this.prefs.autoFullscreen && keepsFull,
          !keepsFull,
        ) +
          (keepsFull
            ? ""
            : `<div class="note">${escapeHtml(t("menu.settings.fullscreenEscape"))}</div>`) +
          `<div class="row lang${inGame ? " off" : ""}">` +
          `<span class="lbl">${escapeHtml(t("menu.settings.language"))}</span>` +
          `<div class="langs strip" data-group="lang">${languages()
            .map((l) => {
              const on = l.code === language();
              return (
                `<button class="bs-menu-btn chip${on ? " on" : ""}" type="button" ` +
          ...
          (inGame
            ? `<div class="note">${escapeHtml(t("menu.settings.languageInGame"))}</div>`
            : "") +
          this.autosaveRow(),
      ) +
      this.section(
        ...
        this.toggle(
          "hapticFeedback",
          t("menu.settings.hapticFeedback"),
          this.prefs.hapticFeedback,
        ) +
  ...
  private autosaveRow(): string {
    ...
    return (
      ...
      `<div class="steps strip" data-group="autosave">${steps
        .map(
          (m) =>
            `<button class="bs-menu-btn chip${m === now ? " on" : ""}" type="button" ` +
            `data-autosave="${m}"${m === rove ? "" : ' tabindex="-1"'}>` +
            `${escapeHtml(t("menu.settings.autosave.minutes", { n: String(m) }))}</button>`,
        )
        .join("")}</div>` +
      `</div></div>`
    );
  ...
  private volumeRow(): string {
    ...
    return (
      ...
      `<div class="steps strip" data-group="vol">${VOLUME_STEPS.slice(1)
        .map(
          (s) =>
            `<button class="bs-menu-btn chip${s === pc ? " on" : ""}" type="button" ` +
            `data-vol="${s}"${s === rove ? "" : ' tabindex="-1"'}>${s}</button>`,
        )
        .join("")}</div>` +
      `</div></div>`
    );
  ...
  handleClick(btn: HTMLElement): boolean {
    ...
    if (toggle === "hapticFeedback") {
      this.hooks.onHapticFeedback(next);
    } else if (toggle === "invertLookX") {
      this.hooks.onLookAxes({ invertX: next });
    } else if (toggle === "invertLookY") {
      this.hooks.onLookAxes({ invertY: next });
    }
  ...
  stepGroup(el: Element | null, dir: -1 | 1): boolean {
    ...
    switch (strip.getAttribute("data-group")) {
      ...
      case "autosave": {
        const steps = AUTOSAVE_STEPS.slice(1);
        const here =
          this.prefs.autosaveMinutes === 0 ? 0 : steps.indexOf(this.prefs.autosaveMinutes);
  ...
  private pill(btn: HTMLElement, on: boolean): void {
    ...
    if (pill) {
      pill.textContent = on ? t("menu.on") : t("menu.off");
    }
  }
  ...
  private toggle(key: ToggleKey, label: string, on: boolean, disabled = false): string {
    return (
      `<button class="bs-menu-btn row" type="button" data-toggle="${key}" ` +
