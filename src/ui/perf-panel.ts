...
export class PerfPanel {
  ...
  constructor(
    ...
    window.addEventListener(
      "wheel",
      (e) => {
        if (!this.open || !(e.target instanceof Node) || !this.el.contains(e.target)) {
          return;
        }
        e.stopPropagation();
      },
  ...
  onKey(code: string): boolean {
    ...
    if (code === "ArrowRight" || code === "ArrowLeft" || code === "Enter" || code === "Space") {
      ...
      if (this.cursor === ROW_TIME) {
        this.cycleTime(step);
      } else if (this.cursor === ROW_STYLE) {
        this.cycleStyle(step);
      } else if (this.cursor === ROW_COLOUR) {
        this.cycleColour(step);
      } else if (this.cursor >= ROW_MOUNT) {
        this.flipMount(this.cursor - ROW_MOUNT);
      } else if (this.cursor >= ROW_PATH_PROFILE) {
        this.cyclePath(this.cursor, step);
      } else {
  ...
  private flipMount(i: number): void {
    ...
    if (kind) {
      this.mounts.set(kind.id, !this.mounts.has(kind.id));
    }
  }
  ...
  private valueLabel(id: keyof GfxSinks): string {
    ...
    if (typeof v === "boolean") {
      return t(v ? "gfx.on" : "gfx.off");
    }
    if (id === "terrainDistance") {
      return t(
        v === 480 ? "gfx.distance.low" : v === 600 ? "gfx.distance.medium" : "gfx.distance.high",
      );
    }
    if (id === "foliageDistance") {
      return t(
        v === 64 ? "gfx.distance.low" : v === 96 ? "gfx.distance.medium" : "gfx.distance.high",
      );
    }
  ...
  private cycleStyle(step: number): void {
    ...
    const i = Math.max(
      0,
      list.findIndex((s) => s.id === this.look.style()),
    );
  ...
  private cycleColour(step: number): void {
    ...
    if (at < 0) {
      ...
      list.forEach((hex, i) => {
        const d =
          Math.abs(((hex >> 16) & 255) - ((now >> 16) & 255)) +
  ...
  private buildShell(): void {
    ...
    this.gfxList?.addEventListener("input", (e) => {
      const well = (e.target as Element | null)?.closest?.(
        ".bs-hair-well",
      ) as HTMLInputElement | null;
      if (well) {
        this.look.setColour(parseInt(well.value.slice(1), 16));
      }
    });
  ...
  private renderGfx(): void {
    ...
    const timeRow =
      `<div class="bs-perf-row${this.cursor === ROW_TIME ? " sel" : ""}"` +
    ...
    this.gfxList!.innerHTML =
      gfxRows + timeRow + this.hairRows() + this.pathRows() + this.mountRows();
  }
  ...
  private hairRows(): string {
    ...
    return (
      `<div class="bs-perf-head">${escapeHtml(t("hair.section"))}</div>` +
  ...
  private cyclePath(row: number, step: number): void {
    if (row === ROW_PATH_PROFILE) {
      ...
      const i = Math.max(
        0,
        list.findIndex((o) => o.id === this.paths.profile()),
      );
  ...
  private pathRows(): string {
    ...
    const row = (cursor: number, key: string, name: string, val: string, cost: string): string =>
      ...
      "</div>";
    return (
      `<div class="bs-perf-head">${escapeHtml(t("path.section"))}</div>` +
      row(
        ROW_PATH_PROFILE,
        "profile",
        t("path.profile"),
        profile ? t(profile.labelKey) : this.paths.profile(),
        t("path.profile.cost"),
      ) +
      row(
        ROW_PATH_LENGTH,
  ...
  private mountRows(): string {
    return (
      `<div class="bs-perf-head">${escapeHtml(t("mount.section"))}</div>` +
      this.mounts.kinds
        .map((k, i) => {
          const on = this.mounts.has(k.id);
          return (
            `<div class="bs-perf-row${this.cursor === ROW_MOUNT + i ? " sel" : ""}` +
  ...
  private branchHtml(b: SpawnBranch, rows: readonly SpawnRow[], openBranch: boolean): string {
    const head =
      `<div class="bs-spawn-branch${openBranch ? " open" : ""}"` +
