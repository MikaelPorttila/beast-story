...
| Regenerate the Act 4 track          | `bun tools/gen-seam-track.mjs` (seeded; writes `src/audio/seam.webm` via ffmpeg)    |
...
| Zones and gateways                  | `world/zones.ts` (the rules), `world/portal.ts` (the arch), `world/dungeon.ts` (the hold and the vent: one shape, two specs), `world/seam.ts` (the finale's disc; its arch is the one a player OPENS — `GateSpec.open`) | `test-gateway`, `test-vent`, `test-seam-zone`, `test-first-bond`           |
...
| Combat, enemies, drops              | `combat/index.ts`, `combat/enemies.ts`, `combat/pickups.ts`                                                                                                                                                          | `test-safezone`, `test-aim-assist`, `test-inventory`, `test-land-guardian` (the bond gate) |
